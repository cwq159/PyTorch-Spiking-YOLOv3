...
def fuse_conv_and_bn(conv, bn):
    ...
    with torch.no_grad():
        # init
        assert isinstance(conv, nn.modules.conv.Conv2d) or isinstance(conv, nn.modules.conv.ConvTranspose2d), f"{conv}"
        if isinstance(conv, nn.modules.conv.Conv2d):
            fusedconv = torch.nn.Conv2d(conv.in_channels,
                                        ...
                                        groups=conv.groups,  # for groups conv
                                        bias=True)
        else:
            fusedconv = torch.nn.ConvTranspose2d(conv.in_channels,
                                                 conv.out_channels,
                                                 kernel_size=conv.kernel_size,
                                                 stride=conv.stride,
                                                 padding=conv.padding,
